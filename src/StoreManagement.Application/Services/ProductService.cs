...
public sealed class ProductService(IProductDataService data) : IProductService
{
    ...
    public async Task<ProductResponse?> UpdateAsync(long id, UpdateProductRequest request, CancellationToken cancellationToken)
    {
        ValidateProduct(request.Name, request.PurchasePrice, request.SellingPrice, request.ReorderLevel, request.ReorderQuantity);
        var product = await data.GetByIdAsync(id, cancellationToken);
        if (product is null) return null;
        if (!await data.CategoryExistsAsync(request.CategoryId, cancellationToken)) throw new KeyNotFoundException("Category was not found or is inactive.");
        if (!string.IsNullOrWhiteSpace(request.Sku) && await data.SkuExistsAsync(request.Sku.Trim(), (long?)id, cancellationToken)) throw new InvalidOperationException("SKU already exists.");
        if (!string.IsNullOrWhiteSpace(request.Barcode) && await data.BarcodeExistsAsync(request.Barcode.Trim(), (long?)id, cancellationToken)) throw new InvalidOperationException("Barcode already exists.");
